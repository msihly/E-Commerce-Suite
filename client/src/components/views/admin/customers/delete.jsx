...
const DeleteAlert = ({ customerId }) => {
    const history = useHistory();

    const dispatch = useDispatch();

    const affectedOrders = useSelector(state => state.orders).filter(o => o.customerId === customerId);

    const confirmDelete = (closeModal) => {
        dispatch(actions.deleteCustomer({ customerId, history }));
        closeModal();
    };

    return (
        <Alert id={DELETE_ALERT_ID} modalClasses="pad-ctn-2 border-red" icon={<Media.TrashcanSVG />} iconClasses="red-2-fill"
            heading={["Delete all references to ", <span className="red-2">{actions.getCustomerFullName(customerId)}</span>, "?"]}
